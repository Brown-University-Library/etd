...
class TestCandidateHome(TestCase, CandidateCreator):
    ...
    def test_candidate_get_checklist_complete(self):
        self._create_candidate()
        checklist = self.candidate.get_checklist()
        checklist.dissertation_fee = timezone.now()
        checklist.bursar_receipt = timezone.now()
        checklist.gradschool_exit_survey = timezone.now()
        checklist.earned_docs_survey = timezone.now()
        checklist.pages_submitted_to_gradschool = timezone.now()
        checklist.save()
        auth_client = get_auth_client()
        response = auth_client.get(reverse('candidate_home'))
